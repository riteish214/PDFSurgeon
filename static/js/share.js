...
function initializeShareForms() {
    ...
    if (textShareForm) {
        textShareForm.addEventListener('submit', function(e) {
            e.preventDefault();
            handleTextShare(new FormData(this));
        });
    }
...
function handleTextShare(formData) {
    const textContent = formData.get('text_content');
    if (!textContent || textContent.trim() === '') {
        showErrorMessage('Please enter text content to share.');
        return;
    }
    ...
    .then(data => {
        ...
        if (data.success) {
            displayShareResult(data);
            resetForm('textShareForm');
        } else {
            showErrorMessage(data.error || 'Failed to share text.');
        }
    })
    .catch(error => {
        hideLoadingModal();
        console.error('Error:', error);
        showErrorMessage('An error occurred while sharing the text.');
    });
}
